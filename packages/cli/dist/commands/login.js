...
import { saveGlobalConfig, getWebAppURL } from '../utils/config.js';
const CALLBACK_PORT = 3333;
const getLoginUrl = () => {
    return `${getWebAppURL()}/cli-login`;
};
export const loginCommand = async (providedApiKey) => {
    if (providedApiKey) {
...
async function verifyAndSave(apiKey, refreshToken) {
