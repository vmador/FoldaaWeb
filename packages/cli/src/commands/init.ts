...
export async function initCommand() {
      ...
      name: () =>
        p.text({
          ...
          validate: (value) => {
            ...
            if (!/^[a-z0-9-]+$/.test(value)) return 'Name can only contain lowercase letters, numbers, and dashes.';
          },
        }),
      framework: () =>
        p.select({
  ...
  const config: FoldaaConfig = {
    name: group.name as string,
    framework: group.framework as string,
    outputDir: group.outputDir as string,
    buildCommand: group.buildCommand as string,
  };
